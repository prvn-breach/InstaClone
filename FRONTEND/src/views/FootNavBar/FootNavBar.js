...
export default (props) => {
    ...
    let routeUrlLength = routeUrl.length;
    const [url, updateUrl] = useState(routeUrl[routeUrlLength-1]);
    return (
        <nav id="foot-navbar" className="navbar fixed-bottom navbar-light border-top bg-white pb-0">
            <div className="d-flex justify-content-between justify-content-center w-100">
                <Link to="/newsfeed" onClick={() => updateUrl("newsfeed")} className="text-dark" >
                    <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" fill="currentColor" className={"bi " + url === "newsfeed" ? "bi-house-door-fill" : "bi-house-door"} viewBox="0 0 16 16">
                        {url === "newsfeed"
                ...
                <Link to={`/profile/${props.user.username}`} className="text-dark">
