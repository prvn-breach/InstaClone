...
export default class Modal extends Component {
    ...
    showModal(post, user_id) { 
        ...
        this.setState({ is_current_user_post: user_id === post.user});
                    ...
                    <div className="modal-dialog a-bounce" role="document">
                        <div className="modal-content p-0 m-0"> 
                            <div className="modal-body text-center p-lg p-0" id="menu_btns">
                                {!this.state.is_current_user_post && <button type="button" className="form-control bg-white p-0 m-0 text-danger font-weight-bold">Unfollow</button>}
                                {this.state.is_current_user_post && <button type="button" onClick={() => this.deletePost(this.post._id)} className="form-control bg-white p-0 m-0 text-danger font-weight-bold">Delete</button>}
